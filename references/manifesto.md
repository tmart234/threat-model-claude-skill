...
- **Generic mitigations.** "Apply RBAC", "use encryption", "implement input validation", "follow least privilege". These describe a control *class*, not a *control*. **Fix:** name the actual mechanism, the actual control surface, the actual party who implements it. *"Tenant scope enforced in middleware on every authenticated route, with a per-route automated test that asserts a request from tenant A cannot read tenant B's data, owned by the API team"* is a mitigation. *"Apply RBAC"* is a placeholder for one. When the control is cryptographic, name the suite (TLS 1.3 / mTLS, AES-256-GCM, Ed25519, HKDF, Argon2id) — *"use encryption"* is the same anti-pattern with a different word. See `references/stride-prompts.md` § "Choosing the right primitive".
- **Single-control rows on high-impact threats.** A `Mitigate` row on a threat that crosses a trust boundary with `C,I,A` impact has one control in the cell, as if defense were one fence to climb. **Fix:** target a layered stack of 2–6 controls per Mitigate row, ordered **Protect first** (the primary class — prevent the threat) with at least one Detect path and one Respond / Recover path on high-impact threats so the design has a fallback when a Protect control fails silently. Prefer built-in mitigations from the in-scope technology (platform IAM, KMS, framework CSRF, OS sandboxing, secure boot) over custom code. See `SKILL.md` § "Mitigations and Q4".
- **Threat cell that isn't a title-plus-description.** Three shapes, all the same failure — the `Threat` column doesn't read easily for a developer, systems engineer, or QE reviewer scanning the table. (a) A row reads `T7 | Auth Service | S | Spoofing | … | N | N | L | C, I` — the cell is just the STRIDE category. (b) The cell is an unstructured sentence with no leading title, so the reader has to parse the whole thing to know what the row is about. (c) The cell is a three-plus-sentence paragraph that buries the point. **Fix:** every threat cell is `**Title**: description.` — a ≤6-word noun-phrase title naming the threat, a colon, then a 1–2 sentence concrete scenario (who, against which element, by what means, to what end). *"**Forged JWT audience claim**: An attacker on the public internet sends a forged JWT with a manipulated `aud` claim to bypass the auth service's audience check."* — not *"Spoofing"*, not the description alone, not a paragraph. See `SKILL.md` § "Threat enumeration".
...
- **Exposure inflation — `AV:N / PR:N / AC:L` on every row.** Every threat is scored as if it were anonymously internet-reachable and trivially repeatable, regardless of whether the element actually is. The §3 sort collapses (everything's at the top) and the TM-BOM `likelihood` field reads `certain` for findings that need a privileged shell to reach. **Fix:** `AV` reflects the actual reach to the element (`A` for on-segment, `L` for host-local, `P` for physical contact); `PR` reflects the role the attacker must already hold (`L` for any authenticated user, `H` for admin); `AC:H` is the *exception*, used only when a precondition genuinely is outside the attacker's control (race window, victim interaction beyond a click). See `references/risk-rating.md` § "Default scheme". When every row scores `AV:N / PR:N / AC:L`, that's a smell — re-check against the DFD's actual reachability.
