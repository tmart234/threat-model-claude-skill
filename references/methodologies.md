...
A variant of attack-tree construction designed to scale to cyber-physical / IoT / IoMT (Internet of Medical Things) environments where naive enumeration explodes combinatorially. Stellios, Kotzanikolaou & Grigoriadis (*Assessing IoT-enabled cyber-physical attack paths against critical systems*, Computers & Security 107, 2021) extend the Agadakos et al. (2017) IoT attack-graph baseline by adding risk-based pruning, so the search yields the small set of paths a defender can act on rather than the full graph. The construction is not IoMT-specific: it generalizes to any multi-device cyber-physical environment where a low-criticality device can be the entry point for a high-criticality target — smart buildings, ICS / SCADA plant floors, automotive fleets, robotics cells, and industrial IoT gateways are equally good fits.
...
| **Medical device / PACS / DICOM** | flow-centric DFD + STRIDE — **for safety-critical control loops (infusion pumps, ventilators, robotic surgery, dialysis), add STPA as swap (FDA / IEC 62304 / IEC 81001-5-1 joint artifact) or as supplement (alongside STRIDE)** — see `stpa.md` | **data-centric (PHI / device data)** + LINDDUN | ATT&CK; **CAPEC + CWE chain** (already on every §2.1 row; use the medical-device subset in `capec.md`; cite Standard-level patterns by default — DICOM-/HL7-specific Detailed patterns don't exist, see `capec.md` § "Honest about CAPEC coverage"); CISA medical advisories | **H-ISAC; FDA premarket cybersecurity; IEC 62443; IEC 81001-5-1; HIPAA; ISO 14971 / AAMI TIR57 mapping from `risk-rating.md` (severity from STPA `H-#` + `P1 × P2` from row's `AV / PR / AC`)** | asset-centric (signing keys); attack tree on safety-critical path; **STPA on safety-critical control loops** | Code-centric on parser/protocol code; named adversary if applicable; **risk-prioritized cyber-physical attack paths (Stellios) for IoMT patient rooms with multiple co-located devices** |
