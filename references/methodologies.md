...
> **Related**: ← `SKILL.md` • `centric-methods.md` (entry points; canonical home for generation-vs-characterization) • `stride-prompts.md` (STRIDE-Per-Element / Per-Interaction) • `capec.md`, `risk-rating.md`, `validation.md` (operational/strategic-stratum supporting refs). Industry-specific decision-matrix rows and worked examples live in the relevant `industries/<industry>/` pack.
...
and you can characterize the adversary?
  → Add an attack tree for that asset.
...
**Compliance touch.** PASTA's stage-1 business-impact framing is occasionally requested by regulators (e.g. FDA / IEC 81001-5-1 reviewers) and enterprise compliance teams as the executive-tied risk view that connects technical findings to revenue / fine / reputational exposure. When that's the ask, bolt PASTA's stage 1 onto the strategic stratum (`§2.3` of the threat-model template) — write the business-impact framing as a §2.3 sub-paragraph and cross-reference the affected threats. Don't redo the contextual or operational work; the rest of PASTA's seven stages largely overlap with what STRIDE + ATT&CK + the risk register already produced.

## Attack trees

A tree where the root is an attacker goal ("steal the firmware signing key", "fraudulently issue a refund", "modify a record after acquisition") and the children are sub-goals or attack steps. Leaves are atomic actions. Branches are AND (all required) or OR (any sufficient).
...
- **Multi-hop cyber-physical paths** where individual elements look fine under per-element STRIDE but their composition is dangerous (one device's RF surface enables another device's safety failure across a room or plant floor). For the construction technique that makes this tractable, see "Risk-prioritized cyber-physical attack paths (Stellios et al., 2021)" below.

**When to be cautious:** attack trees are most at home in environments with named, capability-assessed adversaries — government / classified work, nation-state-target finance, defense contractors. In settings where realistic threats come mostly from commodity attackers, opportunistic ransomware, or insiders rather than characterized APTs, a flow-centric STRIDE pass is usually more productive; reserve attack trees for one or two of the highest-priority threats (e.g. "how could someone forge a firmware signature") where adversarial reasoning genuinely adds value. Don't draw an attack tree per threat — that's a sign the methodology is being applied for its own sake.

The other failure mode — specific to multi-device cyber-physical / IoT environments — is **exponential blow-up without risk-based pruning**: the Agadakos et al. (2017) IoT-attack-graph baseline enumerates every reachable path and quickly produces graphs no team can review. The Stellios et al. (2021) fix is to score each hop with a CVSS-derived metric (CVV) and prune below a threshold *before* expanding the next layer — see "Risk-prioritized cyber-physical attack paths" below. If you're building an attack tree for a multi-device cyber-physical environment, do this from the start; an unpruned graph is not an artifact, it's a wall.
...
A variant of attack-tree construction designed to scale to cyber-physical / IoT environments where naive enumeration explodes combinatorially. Stellios, Kotzanikolaou & Grigoriadis (*Assessing IoT-enabled cyber-physical attack paths against critical systems*, Computers & Security 107, 2021) extend the Agadakos et al. (2017) IoT attack-graph baseline by adding risk-based pruning, so the search yields the small set of paths a defender can act on rather than the full graph. The construction generalizes to any multi-device cyber-physical environment where a low-criticality device can be the entry point for a high-criticality target — smart buildings, ICS / SCADA plant floors, automotive fleets, robotics cells, industrial IoT gateways, and IoMT clinical rooms are equally good fits.

**Two terms used throughout:** *CVV* (Cyber-physical Vulnerability Vector) is a CVSS-derived per-hop risk score extended with physical-interaction parameters; the *P1/P2/P3* taxonomy classifies the between-device physical surface that each hop crosses. CVV is what enables risk-based pruning; P1/P2/P3 is what enables cross-device hop enumeration.
...
- **P1 / P2 / P3 between-device physical-surface taxonomy.** **P1** = direct physical contact (USB, JTAG, an attached cable, a service-port adapter); **P2** = physical proximity within the same controlled space (BLE/NFC reach, line-of-sight IR, audible-range acoustic injection, near-field EM); **P3** = shared physical medium *without* a proximity requirement (the 2.4 GHz ISM band shared across Wi-Fi / BLE / Zigbee in a room, where a malicious BLE peripheral can DoS a Wi-Fi-attached controller without ever associating with it; or a shared power rail, a shared HVAC duct for acoustic side channels). The taxonomy makes "one device's physical surface is another device's threat" explicit and is what enables the cross-device hops that per-element STRIDE misses.
...
Risk rating (`AV / PR / AC + CIA` per row by default, or OWASP-RR / FMEA where the org requires it — see `risk-rating.md`) sits *across* the strata: every threat at every layer carries the same column set so they can be merged into one prioritized list sorted on the same enums.
...
| Embedded / IoT | flow-centric DFD + STRIDE — **for control-loop devices (automotive ECUs, robotics, drones), add STPA as swap (ISO 26262 joint artifact) or supplement** | data-centric (firmware image; device keys); asset-centric | ATT&CK | Sector regulator; supply-chain framing (SBOM, signed firmware) | Code-centric on bootloader / parser; attack tree on firmware-signing bypass | LINDDUN if device collects PII; **STPA for control-loop devices**; **risk-prioritized cyber-physical attack paths (Stellios) when several devices share a physical space (smart building, vehicle cabin, robotics cell) and per-element STRIDE misses cross-device P2/P3 hops** |
...
**Single-document hybrid (default).** All strata in one markdown file, with the document structure from SKILL.md § "Producing the threat model". Section 2 ("What can go wrong?") has up to three subsections — 2.1 Contextual, 2.2 Operational, 2.3 Strategic — each populated when there's content for it. Threats across present strata share a single ID space and the same `AV / PR / AC + CIA` enums so Section 3 ("What are we going to do about it?") can prioritize across them with a single sort. This is what most engineering teams want.
...
In either shape: cross-reference threats across strata by ID. *"V3 (data-centric) ≈ T7 (flow-centric); CAPEC-150 (Standard) → CWE-532; maps to ATT&CK T1119 (Automated Collection); within-sector precedent: sector-ISAC advisory 2023-07."* That's the chain that makes a hybrid actually hybrid rather than three disconnected lists. The CAPEC → CWE step in the middle is what turns the chain from a list of references into something a developer can act on — see `capec.md` for the full mapping by STRIDE category, abstraction-level rule of thumb, and the embedded/ICS subset.
...
A worked end-to-end hybrid example — flow-centric DFD + data-centric pass in the contextual stratum, ATT&CK / CAPEC mapping in the operational stratum, sector-ISAC and regulatory references in the strategic stratum — is bundled with each industry pack under `industries/<industry>/worked-examples.md`.
