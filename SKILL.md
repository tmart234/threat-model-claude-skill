...
- `risk-rating.md` — Qualitative L/M/H, OWASP-RR pointer, MITRE CVSS Rubric for Medical Devices pointer, ISO 14971 / TIR57 mapping, safety-bump rule, why DREAD is discouraged.
...
- **MITRE** — ATT&CK, CAPEC, CWE, **D3FEND** (https://d3fend.mitre.org/ — defensive-technique knowledge graph mapped to ATT&CK techniques; closes the offensive→defensive loop in the operational stratum), **CVSS Rubric for Medical Devices** (https://mitre.github.io/md-cvss-rubric-tools/ — branching decision tree for CVSS scoring in clinical context; the FDA-aligned scoring rubric for medical-device CVE / submission work).
