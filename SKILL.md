...
| `risks[]` | Derived at TM-BOM emission from §2.1 row's `AV / PR / AC / Impact` columns, one risk per threat (or per cluster of cross-referenced threats) | `threats` array of refs; `likelihood` derived from AV+PR+AC; `impact` derived from CIA-impact set; `score` is integer 0–25; `level` enum: `very_low / low / medium / high / very_high / critical`. Translation table: `references/risk-rating.md` § "Translation to TM-BOM enums" |
