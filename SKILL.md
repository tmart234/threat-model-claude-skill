...
- **Trust boundary** → Mermaid `subgraph` with a structured label that names environment owner, environment type, and trust level: `subgraph HospNet["Hospital IT | on-prem enterprise | moderate trust"]`. Use this convention consistently — see `references/dfd-mermaid.md` § "Subgraph labeling convention". For the boundary line itself, add a note in prose ("dotted line between subgraphs = trust boundary") since Mermaid doesn't render a true threat-modeling trust boundary. The per-environment patterns that should drive *which* subgraphs you draw (Purdue levels for OT, app sandbox / keychain for mobile, secure-boot / JTAG for embedded, account / VPC / IAM for cloud, AD forest / SSO / jump-host for enterprise) live in `references/environments.md` — read it after the system-type matrix when drawing the DFD.
...
The "default = single-method, swap if needed" framing is gone. The question is no longer *"do I swap STRIDE / flow-centric for something else?"* — the answer is almost always no for the contextual core. The one well-documented exception is **STPA-SafeSec** for safety-critical control-loop systems where the worst case is physical harm (medical devices with control loops, ICS / SCADA / OT, automotive, aerospace, robotics) — there, swap the contextual core; see `references/stpa-safesec.md`. Otherwise, the right question is *"which additional layers does the system in front of me need in its hybrid?"*. Use the applicability matrix in `references/methodologies.md` § "Decision matrix — which layers, by system type" as the starting point.
...
- **STPA-SafeSec (contextual-core swap, not a supplement)** — for safety-critical control-loop systems where the worst case is physical harm. Replaces flow-centric DFD + STRIDE for the control-loop portion; STRIDE may still layer in for non-control flows (telemetry, OTA, audit) at the component layer. Workflow: `references/stpa-safesec.md`.
...
- `references/stpa-safesec.md` — **STPA-SafeSec deep dive**: full-methodology alternative to flow-centric + STRIDE for safety-critical control-loop systems. Losses → hazards → constraints → control layer → component layer → UCAs → system flaws → hazard-scenario trees → mitigations + requirements traced to constraints. Use when the worst case is physical harm and the system is shaped as a control loop (medical devices, ICS / SCADA / OT, automotive, aerospace, robotics).
